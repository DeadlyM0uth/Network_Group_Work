...
Для этого использовалась отправка статических данных, созданной ранее учетной записи, через аргумент строки команды. Утилита циклически отправляла POST-запросы на эндпоинт авторизации: `/_matrix/client/v3/login` в 100 параллельных потоков непрерывно в течение 1 минуты.

```bash
hey -z 1m -c 100 -m POST -T "application/json" -d '{"type":"m.login.password","identifier":{"type":"m.id.user","user":"bodya"},"password":"inekrass119"}' http://100.83.165.96:8008/_matrix/client/v3/login
```
...
Из-за статичности передаваемых в команде данных сервер быстро активировал встроенный механизм ограничения частоты запросов (Rate Limiting) и начал отбивать последующие атаки легкой ошибкой `HTTP 429 Too Many Requests`, что привело к резкому падению нагрузки на CPU.
...
- Если при использовании `hey` сервер отдавал стандартный ответ `HTTP 429`, блокируя одного пользователя без затрат вычислительных ресурсов, то в сценарии `k6` ситуация была иной. Из-за динамической генерации имен `k6` заставил сервер прогнать каждый из 69 324 запросов через полный цикл хэширования. Это привело к стопроцентной перегрузке процессора и росту времени обработки запросов до 15 секунд.

Более того, в тесте `k6` сервер вероятно возвращал не ошибку ограничения частоты (429), а код `HTTP 401 Unauthorized`. Так как запросы отправлялись от лица несуществующих в базе данных пользователей, что вынуждало систему каждый раз выполнять ресурсоемкую валидацию перед тем, как отказать в доступе. Таким образом, был достигнут аппаратный отказ в обслуживании (DoS).